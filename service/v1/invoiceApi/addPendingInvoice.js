...
module.exports = {
  rest: {
    rpc: 'invoiceApi.pending.add',
    path: '/v1/invoice/pending/add',
    config: {
      ...
      validate: {
        payload: joi.object({
          ...
          info: joi.string().description('Pending invoice description').example('Invoice in the amount of $25 for prepaid TV')
        })
      },
      plugins: {
        'hapi-swagger': {
          responses: {
            '200': {
              description: 'Pending invoice added',
  ...
  'pending.add': function (msg, $meta) {
    msg.invoiceType = 'pending'
